...
class UserDash extends Component {
    ...
    componentDidMount() {
        ...
        UserAPI.getPage("admin", pageNum, pageSize)
        .then(response => {
            console.log("response");
            console.log(response.data);
            let payload = response.data;
            let users = payload.content;
    ...
    displayListings() {
        return this.state.users.map((user, index) => {
            ...
            let active = (user.active === true) ? "active" : "inactive";

            return (
                <tr key={index.toString()}>
                    <td><a href={"/user/read?uid="+user.uid}>{user.id}</a></td>
